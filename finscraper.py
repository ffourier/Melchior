...
class Form10K_Data_Extractor:
	...
	def print_f10k_links(self):
		...
		'''Prints out link to each 10-K form filed by the company corresponding to self._ticker'''
	...
	def print_net_incomes(self):
		...
		'''Prints out the historical net income data for the company corresponding to self._ticker'''		
			...
			if "txt" in link: # If dealing with txt files, use regex only
				...
				for item in f10k_file.text.split("\n"):
					...
					if not incomes_captured and re.match(ni_txt_regex, item):
						ni_str = item.replace("$", "")
						ni_str = re.sub(r"\s*Net (income|loss)+\s*(\(loss\))*\.*", "", ni_str)
						ni_str = ni_str.replace(",", "")
						ni_str = ni_str.replace("(", "-").replace(")", "") 
						net_incomes = ni_str.split()
						net_incomes = list(map(int, net_incomes))	
